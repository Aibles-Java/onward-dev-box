...
make db-reset  # truncate feature_flag_db to a clean slate (app stays up; no volume wipe)
make seed      # seed demo data into a running feature_flag via its Admin API
make smoke     # reset the DB, then run feature_flag's collection (needs `make run` live)
make sonar     # local SonarQube analysis of feature_flag + quality-gate verdict
...
the running app. The collection registers fixed demo users, so it needs a clean DB —
`make smoke` **truncates the DB for you first** (via `make db-reset`), so you can re-run
it repeatedly without a full `make nuke`:

```bash
make run       # one terminal: app on the host (ups + waits for core DB)
make smoke     # another terminal: resets the DB, then runs the collection via newman
```

`make smoke` is repeatable — each run starts from a clean slate. Pass `SMOKE_NO_RESET=1
make smoke` to run against the current DB state without truncating. You only need
`make nuke` for a full volume wipe (e.g. after a `feature_flag` schema change), not for
a normal smoke run.
...
| App starts but auth/data is stale | dirty DB from a previous run | `make db-reset` (truncates, app stays up), or `make nuke` for a full volume wipe |
